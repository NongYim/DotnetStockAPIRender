...
public class AuthenticateController:ControllerBase{
    ...
    //สร้าง constructor สำหรับ initial ค่าของ ApplicationDbContext
    ...
    public void testconnectdb()
    {
        ...
        //     //ถ้าเชื่อต่อได้จะแสดงข้อความ 
        //     Response.WriteAsync("Connected");
        // }else{
            
        //     Response.WriteAsync("Not Cpnnected");
        // }
        try{
    ...
    public async Task<ActionResult> RegisterUser([FromBody] RegisterModel model)
    {
        ...
        if (!await _roleManager.RoleExistsAsync(UserRolesModel.Admin)) //ไม่ได้เป็น admin มาก่อน
        {
            await _roleManager.CreateAsync(new IdentityRole(UserRolesModel.Admin));
        }
        //ถ้าไม่มี role Manager ให้สร้าง Manager  ใหม่

        if (!await _roleManager.RoleExistsAsync(UserRolesModel.Manager)) //ไม่ได้เป็น Manager มาก่อน
        {
            await _roleManager.CreateAsync(new IdentityRole(UserRolesModel.Manager));
        }
        //ถ้าไม่มี role User ให้สร้าง User  ใหม่ และเพิ่ม user ลงใน role user

        if (!await _roleManager.RoleExistsAsync(UserRolesModel.User)) //ไม่ได้เป็น User มาก่อน
        {
            await _roleManager.CreateAsync(new IdentityRole(UserRolesModel.User)); //สร้าง user
            await _userManager.AddToRoleAsync(user, UserRolesModel.User); // เพิ่มสิทธ เป็น user
        }else{
  ...
  // ฟังก์ชันสร้าง Token
    ...
    public async Task<IActionResult> Logout()
    {
        ...
        if (userName != null)
        {
            ...
            if (user != null)
            {
                await _userManager.UpdateSecurityStampAsync(user); //ไปเปลี่ยน token ใหม่ ทำให้ไม่สามารถใช้ token เดิมได้
