...
public class ProductController:ControllerBase
{
    ...
    //wwwroot ต้องใช้ชื่อนี้ ถึงจะใช้ พวกไฟล์ ตอนไป public ได้
    ...
    )
    {
        ...
        int skip = (page - 1) * limit; ///คือเลขหน้า-1 * limit  ถ้าหน้าที่ 2 คือจะเริ่ม ดาต้าตัว 100 หน้าต่อไป เริ่มที่ 200
        ...
        {
            //ILike คือจะไม่สนใจว่าพิมตัวเล็กตัวใหญ่ จะเหมือนกัน เช่น iphon Iphone คืออันเดียวกัน
            //Like  จะไม่เหมือนกัน พิมตัวเล็กตัวใหญ่ ถือคนหล่ะคำ

            //sqlserver จะไม่มี ILike  ถ้า pg จะมี ส่วน sqlserver Like จะไม่สนตัวเล็กหรือใหญ่ คือเหมือนกัน
            query = query.Where(p => EF.Functions.Like(p.productname!, $"%{searchQuery}%"));
        }
    ...
    public async Task<ActionResult<product>> CreateProduct([FromForm] product product, IFormFile? image)
    {
        ...
        if(image != null)
        {
            ...
            //Guid.NewGuid().ToString() จะได้เลขไม่ซ้ำ
  ...
  // ฟังก์ชันสำหรับการแก้ไขข้อมูลสินค้า
    ...
    public ActionResult<product> DeleteProduct(int id)
    {
        ...
        // ลบข้อมูล
        _context.product.Remove(product); // delete from products where id = 1
